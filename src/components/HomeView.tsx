...
export default function HomeView({ onNavigate, onStartPractice }: HomeViewProps) {
  ...
  return (
    <div className="space-y-6 sm:space-y-10 animate-pageFadeIn">
      {/* Hero Welcome Card */}
      <div className="relative overflow-hidden bg-vibrant-indigo rounded-[2rem] sm:rounded-[3rem] p-6 sm:p-12 text-white shadow-2xl shadow-vibrant-indigo/20">
        ...
        <div className="relative z-10 space-y-4 sm:space-y-6 max-w-2xl">
          ...
          <div className="pt-2 sm:pt-4 flex flex-col sm:flex-row gap-3 sm:gap-4">
            <button
              ...
              className="flex items-center justify-center gap-2 bg-vibrant-coral hover:bg-vibrant-coral/90 text-white px-6 sm:px-7 py-3 sm:py-3.5 rounded-2xl font-black shadow-lg shadow-vibrant-coral/20 uppercase tracking-tight transition-all active:scale-95 cursor-pointer text-sm"
            >
              <PlusCircle size={18} />
              Viết Nhật Ký Mới
            </button>
            <button
              ...
              className="flex items-center justify-center gap-2 bg-vibrant-mint hover:bg-vibrant-mint/90 text-slate-900 px-6 sm:px-7 py-3 sm:py-3.5 rounded-2xl font-black shadow-md transition-all active:scale-95 cursor-pointer text-sm"
            >
              <BookOpen size={18} />
              Thư Viện Chunks
            </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8">
        ...
        <div className="lg:col-span-7 bg-white p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6">
            ...
            <div className="pt-4 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-3 text-[10px] sm:text-xs text-slate-500 font-medium">
              ...
              {chunks.filter(c => (c.stars || 0) > 0 && (c.stars || 0) < 4).length > 0 && (
                <button
                  ...
                  className="flex items-center gap-1.5 text-vibrant-indigo hover:text-vibrant-indigo/80 font-black cursor-pointer bg-vibrant-indigo/5 px-4 py-2 rounded-xl transition-all w-full sm:w-auto justify-center"
                >
                  <PlayCircle size={14} />
        ...
        <div className="lg:col-span-5 bg-white p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col justify-between space-y-6">
          <div className="space-y-4">
            <div className="space-y-1">
              <h2 className="font-display text-lg sm:text-xl font-black text-slate-900">Ngôn ngữ học</h2>
              <p className="text-[10px] sm:text-xs text-slate-500 font-medium">Cấu hình trong Cài Đặt.</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 pt-2">
              {settings?.learningLanguages.map((lang) => {
                ...
                return (
                  <div
                    key={lang}
                    className="bg-slate-50 hover:bg-slate-100/80 border border-slate-100 p-3 sm:p-4 rounded-2xl text-center transition-all"
                  >
                    <Languages className="mx-auto text-vibrant-indigo mb-1.5" size={20} />
                    <span className="block font-bold text-slate-800 text-xs sm:text-sm">{lang}</span>
                    <span className="font-mono text-[10px] text-slate-400">{count} chunks</span>
                  </div>
                );
              })}
              {(!settings || settings.learningLanguages.length === 0) && (
                <div className="col-span-2 text-center py-6 text-slate-400 text-[10px] sm:text-sm italic">
                  Chưa chọn ngôn ngữ.
                </div>
              )}
            </div>
          ...
          <button
            ...
            className="w-full py-3 sm:py-3.5 text-center bg-slate-100 hover:bg-slate-200/80 text-slate-700 font-bold text-xs rounded-2xl transition-all cursor-pointer mt-auto"
          >
            Thay Đổi Ngôn Ngữ
          </button>
        </div>
      ...
      {/* Philosophy Callout */}
      <div className="bg-vibrant-mint rounded-2xl sm:rounded-3xl p-5 sm:p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
